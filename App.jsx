...
function AIPanel({onClose}){
  ...
  return(
    <div style={{position:"fixed",right:0,top:0,bottom:0,width:380,background:C.white,borderLeft:`1px solid ${C.border}`,display:"flex",flexDirection:"column",zIndex:100,boxShadow:"-8px 0 32px rgba(78,126,247,0.1)"}}>
      <div style={{padding:"14px 16px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center",background:C.blueL}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{width:36,height:36,borderRadius:11,background:`linear-gradient(135deg,${C.blue},${C.pink})`,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:13,color:C.white,boxShadow:"0 4px 12px rgba(78,126,247,0.35)"}}>AI</div>
          <div>
            <div style={{fontWeight:800,fontSize:13,color:C.text}}>Growth Head AI</div>
            <div style={{fontSize:10,color:C.muted}}>Live data · All platforms connected</div>
          </div>
        </div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:16,background:C.bg}}>
        {msgs.map((m,i)=>(
          <div key={i} style={{marginBottom:14,display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start",alignItems:"flex-start",gap:8}}>
            ...
            <div style={{maxWidth:"86%",padding:"11px 14px",borderRadius:m.role==="user"?"14px 4px 14px 14px":"4px 14px 14px 14px",background:m.role==="user"?`linear-gradient(135deg,${C.blue},#6B9CF8)`:C.white,border:m.role==="user"?"none":`1px solid ${C.border}`,color:m.role==="user"?C.white:C.text,boxShadow:m.role==="user"?"0 4px 16px rgba(78,126,247,.25)":"0 2px 8px rgba(0,0,0,.04)"}}>
      ...
      <div style={{padding:"10px 16px 16px",background:C.white}}>
        <div style={{display:"flex",gap:8,background:C.bg,border:`1px solid ${C.border}`,borderRadius:11,padding:"9px 13px"}}>
          ...
          <button onClick={()=>input.trim()&&!loading&&send(input.trim())} disabled={!input.trim()||loading} style={{width:32,height:32,borderRadius:9,background:input.trim()?C.blue:C.border,border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"background .2s"}}>
...
export default function App(){
  ...
  return(
    <div style={{fontFamily:"'Nunito','Segoe UI',sans-serif",background:C.bg,minHeight:"100vh",color:C.text,paddingRight:aiOpen?380:0,transition:"padding-right .3s"}}>
      <style>{`
      ...
      <div style={{background:C.white,borderBottom:`1px solid ${C.border}`,padding:"0 24px",display:"flex",alignItems:"center",justifyContent:"space-between",height:58,position:"sticky",top:0,zIndex:50,boxShadow:"0 2px 10px rgba(13,18,38,.05)"}}>
        <div style={{display:"flex",alignItems:"center",gap:22}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:38,height:38,borderRadius:11,background:`linear-gradient(135deg,${C.blue},${C.pink})`,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:16,color:C.white,boxShadow:"0 4px 14px rgba(78,126,247,.3)"}}>R</div>
            <div>
              <div style={{fontWeight:900,fontSize:15,letterSpacing:-.4,color:C.text}}>Reprise</div>
              <div style={{fontSize:9,color:C.muted,letterSpacing:.6,fontWeight:600}}>ADS PLATFORM</div>
            </div>
          </div>
          <div style={{width:1,height:28,background:C.border}}/>
          {TABS.map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)} style={{background:"none",border:"none",cursor:"pointer",fontSize:13,fontWeight:tab===t.id?800:500,color:tab===t.id?C.blue:C.muted,padding:"4px 2px",borderBottom:tab===t.id?`2.5px solid ${C.blue}`:"2.5px solid transparent",transition:"all .2s",fontFamily:"inherit"}}>
              {t.label}
            </button>
          ))}
        </div>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{display:"flex",alignItems:"center",gap:6,padding:"6px 12px",background:C.greenL,borderRadius:8}}>
            <div style={{width:7,height:7,borderRadius:"50%",background:C.green,boxShadow:`0 0 6px ${C.green}88`}}/>
          ...
          <button onClick={()=>setAiOpen(!aiOpen)} style={{padding:"8px 18px",background:aiOpen?C.blue:C.blueL,border:`1.5px solid ${C.blue}44`,borderRadius:10,color:aiOpen?C.white:C.blue,fontSize:12,cursor:"pointer",fontWeight:800,fontFamily:"inherit",transition:"all .2s"}}>
            ⚡ AI Growth Head
          </button>
        </div>
      </div>
      ...
      <div style={{padding:"22px 24px 70px",animation:"fadeUp .3s ease"}}>
        ...
        {tab==="overview"&&<>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20}}>
            <Card>
              <div style={{fontSize:11,color:C.muted,letterSpacing:.7,fontWeight:700,marginBottom:14,textTransform:"uppercase"}}>ROAS Trend — 14 Days</div>
            ...
            <Card>
              <div style={{fontSize:11,color:C.muted,letterSpacing:.7,fontWeight:700,marginBottom:14,textTransform:"uppercase"}}>Spend vs Revenue — 14 Days</div>
              <ResponsiveContainer width="100%" height={170}>
                <BarChart data={SPEND_TREND} barGap={2}>
                  ...
                  <Bar dataKey="rev"   fill={C.green} name="Revenue" radius={[5,5,0,0]} fillOpacity={.85}/>
                </BarChart>
              </ResponsiveContainer>
            </Card>
          </div>
          ...
          <Card>
            ...
            {alerts.slice(0,3).map((a,i)=>(
              <div key={a.id} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 0",borderBottom:i<2?`1px solid ${C.border}`:"none"}}>
                <div style={{width:9,height:9,borderRadius:"50%",background:ac(a.type),flexShrink:0,boxShadow:`0 0 7px ${ac(a.type)}77`}}/>
        ...
        {tab==="campaigns"&&(
          <Card style={{padding:0,overflow:"hidden"}}>
            <div style={{padding:"14px 20px",borderBottom:`1px solid ${C.border}`,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <div style={{fontSize:11,color:C.muted,fontWeight:600}}>Sorted by ROAS ↓</div>
            </div>
            <div style={{overflowX:"auto"}}>
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                <thead>
                  <tr style={{background:C.soft,borderBottom:`1px solid ${C.border}`}}>
                    {["Campaign","Platform","Status","Spend","Revenue","ROAS","CAC","CTR","CVR","Fatigue","Action"].map(h=>(
                      <th key={h} style={{padding:"10px 14px",fontSize:9,color:C.muted,letterSpacing:.6,textAlign:h==="Campaign"?"left":"center",fontWeight:700,textTransform:"uppercase"}}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {camps.sort((a,b)=>b.roas-a.roas).map(c=>(
                    <tr key={c.id} className="trow" style={{borderBottom:`1px solid ${C.border}`,transition:"background .15s"}}>
                      ...
                      <td style={{padding:"12px 14px",textAlign:"center",fontSize:12,color:C.muted,fontWeight:600}}>{c.ctr}%</td>
                      <td style={{padding:"12px 14px",textAlign:"center",fontSize:12,color:C.muted,fontWeight:600}}>{c.cvr}%</td>
        ...
        {tab==="creatives"&&(
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:14}}>
            {creas.sort((a,b)=>b.score-a.score).map(c=>{
              ...
              return(
                <Card key={c.id} style={{borderTop:`3px solid ${s}`,transition:"all .25s"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14}}>
                    ...
                    <div style={{textAlign:"center",background:c.score>=80?C.greenL:c.score>=60?C.yellowL:C.redL,borderRadius:12,padding:"8px 12px",minWidth:54}}>
                      ...
                      <div style={{fontSize:8,color:C.muted,fontWeight:700,letterSpacing:.6}}>SCORE</div>
                    </div>
                  </div>
        ...
        {tab==="alerts"&&(
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {alerts.length===0&&<div style={{textAlign:"center",padding:60,color:C.green,fontSize:14,fontWeight:700,background:C.white,borderRadius:14,border:`1px solid ${C.border}`}}>✓ No active alerts — all systems healthy</div>}
            {alerts.map(a=>(
              <div key={a.id} style={{background:C.white,border:`1px solid ${ac(a.type)}33`,borderLeft:`4px solid ${ac(a.type)}`,borderRadius:12,padding:"14px 18px",display:"flex",alignItems:"center",gap:14,boxShadow:"0 2px 10px rgba(0,0,0,.04)"}}>
                ...
                <button onClick={()=>setDismissed(p=>[...p,a.id])} style={{background:C.soft,border:`1px solid ${C.border}`,borderRadius:8,color:C.muted,cursor:"pointer",fontSize:14,width:30,height:30,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700}}>×</button>
              </div>
            ))}
          </div>
        )}
        ...
        {tab==="budget"&&(
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
            <Card>
              <div style={{fontSize:11,color:C.muted,letterSpacing:.7,fontWeight:700,marginBottom:18,textTransform:"uppercase"}}>Current Allocation</div>
              {Object.entries(PLATFORMS).map(([key,p])=>{
                  ...
                  <div key={key} style={{marginBottom:16}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
                      ...
                      <div style={{display:"flex",gap:12,fontSize:12,alignItems:"center"}}>
                        <span style={{color:C.muted,fontWeight:600}}>{fmt(p.spend)}</span>
                        <span style={{fontWeight:800,color:rc(p.roas)}}>{p.roas}x</span>
                        <span style={{color:C.muted,fontWeight:700}}>{pct}%</span>
                      </div>
                    </div>
                    <div style={{height:9,background:C.soft,borderRadius:5,overflow:"hidden"}}>
                      <div style={{width:`${pct}%`,height:"100%",background:p.color,borderRadius:5,transition:"width .6s",opacity:.85}}/>
                    </div>
                  </div>
                );
              })}
            ...
            <Card>
              <div style={{fontSize:11,color:C.muted,letterSpacing:.7,fontWeight:700,marginBottom:18,textTransform:"uppercase"}}>AI Recommended Allocation</div>
              {[{name:"Meta",pct:58,reason:"Best ROAS + scale headroom",color:C.blue,change:"+6%",up:true},{name:"Google",pct:30,reason:"Strong branded search ROI",color:C.green,change:"+4%",up:true},{name:"Snapchat",pct:8,reason:"Hold — fix CAC first",color:"#E6A817",change:"-2%",up:false},{name:"TikTok",pct:4,reason:"Reduce — below threshold",color:C.pink,change:"-8%",up:false}].map(p=>(
                <div key={p.name} style={{marginBottom:16}}>
                  ...
                  <div style={{height:9,background:C.soft,borderRadius:5,overflow:"hidden"}}>
                    <div style={{width:`${p.pct}%`,height:"100%",background:p.color,borderRadius:5,opacity:.8}}/>
                  </div>
                </div>
              ))}
              <button onClick={()=>setAiOpen(true)} style={{width:"100%",marginTop:8,padding:"11px",background:C.blueL,border:`1.5px solid ${C.blue}33`,borderRadius:10,color:C.blue,fontSize:13,cursor:"pointer",fontWeight:800,fontFamily:"inherit"}}>Get Full Budget Breakdown from AI →</button>
            </Card>
          </div>
        )}
      </div>
